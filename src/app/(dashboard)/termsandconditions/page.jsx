...
const TermsPage = () => {
    ...
    const fetchPolicy = async () => {
      try {
        ...
        if (response?.status == 200) {
          setContent(response.data.data.termsAndConditions)
        } else {
          setContent('')
        }
      } catch (error) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const result = await createPrivacyPolicy(data)

      console.log('tt', result)
      toast.success('Terms & COndition Created!')
      setIsEditing(false)
    } catch (error) {
      console.error('Error creating terms & condition:', error)
      toast.error('Error terms & condition')
    } finally {
      setSubmitting(false)
    }
  }, [content])
